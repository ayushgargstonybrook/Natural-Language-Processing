...
def generate_batch(data, batch_size, num_skips, skip_window):
  ...
  if(data_index+skip_window<len(data)):
  ...
  for i in range(batch_size//num_skips):
      for n in range(num_skips):
        batch[num_skips*i+n]=data[data_index]
        if(n%2==0):
            labels[num_skips * i + n, 0] = data[data_index-(n//2)-1]
        else:
            labels[num_skips * i + n, 0] = data[data_index+(n//2)+1]
...
if __name__ == '__main__':
  ...
  # batch, labels = generate_batch(data, batch_size=8, num_skips=4, skip_window=4)
  # for i in range(8):
  ...
  #Changing the batch size to 64, skip_window to 1 and num_skips to 2 gave me the best results for my model.
  batch_size = 64
